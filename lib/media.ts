...
const seededMediaPexelsIds: Record<string, number> = {
...
export function getSeededMediaFallbackUrl(url: MediaUrl): string | undefined {
  ...
  try {
    ...
    const pexelsId = filename ? seededMediaPexelsIds[filename] : undefined

    return pexelsId
      ? `${PEXELS_BASE}/${pexelsId}/pexels-photo-${pexelsId}.jpeg?auto=compress&cs=tinysrgb&w=1200&h=900&fit=crop`
      : undefined
  } catch {
    return undefined
  }
